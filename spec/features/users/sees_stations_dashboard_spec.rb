...
describe 'visiting the /stations-dashboard' do
  ...
  xit 'sees most bikes available at a station' do
    expect(page).to have_content("Most Bikes Available Count: #{Station.highest_bike_total}")
  end
  ...
  xit 'sees fewest bikes available at a station' do
    expect(page).to have_content("Fewest Bikes Available Count: #{Station.lowest_bike_total}")
  end

  xit 'sees the station that has the fewest bikes available' do
    expect(page).to have_content("Station with Fewest Bikes Available: #{Station.with_lowest_bike_total}")
  end
